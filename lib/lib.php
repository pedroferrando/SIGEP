...
if (ereg("Win32", $_SERVER["SERVER_SOFTWARE"]) || ereg("Microsoft", $_SERVER["SERVER_SOFTWARE"]))
...
$db->cacheSecs = 3600;
$result = $db->Execute("SET search_path=" . join(",", $db_schemas)) or die($db->
                        ErrorMsg());
$db->Execute("SET CLIENT_ENCODING TO Latin1");
unset($result);
$db->debug = $db_debug;
...
$permisos = new gacl(array("items_per_page" => 500, "max_search_return_items" =>
    500));
...
// Funcion que verifica el estado de la sesi�n
function Autenticar() {
    //    global $bgcolor1;
    $status = phpss_auth();
    ...
    switch ($status) {
        ...
        case PHPSS_AUTH_NOCOOKIE:
            //			   Error("Necesita iniciar sesi�n para poder ver esta p�gina");
        ...
        case PHPSS_AUTH_INVKEY:
            phpss_logout();
            //			 Error("Usted est� usando una sesi�n no v�lida");
            //			 include_once(ROOT_DIR."/login.php");
            form_login();
            exit;
            break;
        case PHPSS_AUTH_IPACCESS_DENY:
            //			 Error("Usted no tiene permitido el acceso desde su direcci�n IP");
            //			 include_once(ROOT_DIR."/login.php");
            form_login();
            exit;
            break;
        case PHPSS_AUTH_ACLDENY:
            Error("Usted no tiene permiso para ver esta p�gina");
            exit;
            break;
        case PHPSS_AUTH_HIJACK:
            //			 Error("Su direcci�n IP es diferente a la que uso para iniciar sesi�n");
            //			 include_once(ROOT_DIR."/login.php");
            form_login();
            exit;
            break;
        case PHPSS_AUTH_TIMEOUT:
            ...
            //				 Error("Su sesi�n ha expirado, por favor vuelva a iniciar sesi�n");
            //			 }
            //			 include_once(ROOT_DIR."/login.php");
            form_login();
            exit;
...
function contraste($fondo, $frente, $reemplazo) {
    ...
    $dif = max(($fg_r - $bg_r), ($bg_r - $bg_r)) + max(($fg_g - $bg_g), ($bg_g - $fg_g)) +
            max(($fg_b - $bg_b), ($bg_b - $fg_b));
 ...
 * @desc Esta funcion genera el formulario de busqueda y divide el resultado
  ...
  $sql_tmp = "SELECT IdProv,Proveedor,Contacto,Mail,Tel�fono,Comentarios FROM bancos.proveedores";
...
function form_busqueda($sql, $orden, $filtro, $link_pagina, $where_extra = "", $contar =
 0, $sumas = "", $ignorar = "", $seleccion = "", $limite = 1) {
    // Limite, presetea el paginados quitando el límite
    ...
    if ("$contar" == "buscar") {
        ...
        if ($sumas != '' && substr_count($sql, $sumas["campo"]) > 0 &&
                //si el campo esta definido
                is_array($sumas["mask"]) //mascara para configurar el resultado
        ) {
            $count_mask = count($sumas["mask"]); //tama�o de la mascara
            ...
            elseif (substr_count($sql, $sumas["moneda"]) > 0) { //otro caso //si la moneda esta definida
                ...
                $res_moneda = sql($sql_moneda, "Imposible obtener el listado de moneda") or
                        fin_pagina();
    ...
    if ($page > 0) {
        ...
        $link_pagina_p = "<a title='Pagina anterior' href='" . encode_link($_SERVER["SCRIPT_NAME"], $link_pagina) . "'><<</a>";
    }
    $sum = 0;
    if (($total % $itemspp) > 0)
        $sum = 1;
    ...
    $link_pagina_num = "&nbsp;&nbsp;Pagina&nbsp;<input type='text' value=" . ($page +
            1) . " name='page' size=2 style='text-align:right;border:none' onkeypress=\" if ((show_alert=(window.event.keyCode==13)) && parseInt(this.value)>0 && parseInt(this.value)<= $last_page ) {location.href='" .
            encode_link($_SERVER["SCRIPT_NAME"], $link_pagina) .
            "&page='+parseInt(this.value);return false;} else if (show_alert) {alert('Por favor ingrese un n&uacute;mero v&aacute;lido'); return false;} \" />&nbsp;de&nbsp;$last_page&nbsp;&nbsp;";
    if ($total > $page_n * $itemspp) {
        $link_pagina["page"] = $page_n;
        $link_pagina_n = "<a title='P&aacute;gina siguiente' href='" . encode_link($_SERVER["SCRIPT_NAME"], $link_pagina) . "'>>></a>";
    }
...
function link_calendario($control_pos, $control_dat = "") {
    ...
    return "<script  src='$html_root/lib/popcalendar_1.js'></script> <img src=$html_root/imagenes/cal.gif border=0 align=middle style='cursor:pointer;' alt='Haga click aqui para\nseleccionar la fecha'  onClick=\"javascript:popUpCalendar($control_pos, $control_dat, 'dd/mm/yyyy');\">";
}
...
//funcion defectuosa
//cuidado
function Fecha_db($fecha, $default = '') {
    if (strstr($fecha, "/"))
...
//Igual que la de arriba, pero devuelva ademas la hora timestamp.
function FechaHora_db($fecha, $default = '') {
    if (strstr($fecha, "/"))
        list($d, $m, $a) = explode("/", $fecha);
    elseif (strstr($fecha, "-"))
        list($d, $m, $a) = explode("-", $fecha);
...
/*!
 * Convierte siempre al formato que usa la BD AAAA-MM-DD
 * El formato de la fecha de entrada puede ser AAAA-MM-DD,AAAA/MM/DD,DD/MM/AAAA,DD-MM-AAAA
 * \author G.Fernandez
 * \date 01/10/2013
 */

function ConvFechaComoDB($fecha) {
...
function tr_tag($dblclick, $extra = "", $bgcolor_out_int = '#B7C7D0') {
    ...
    $tr_hover_on = atrib_tr($bgcolor_out_int) . " onClick=\"$target" .
            "location.href ='$dblclick'\"";
...
function sql($sql, $error = -1, $transaccion = -1) {
    ...
    if ($msg) {
        //    enviar_mail($para,$paracc,$parabcc,$asunto,$contenido,$adjunto,$path,$htmlflag=1)
        /* enviar_mail('prescott04@hotmail.com', 'roberto.erhard@gmail.com', '',
          'ERROR - PLAN NACER', $error . ' - ' . $msg . ' - ' . $sql, '', '', 0); */
        //    mail('roberto.erhard@gmail.com', 'ERROR - PLAN NACER', $error.' - '.$msg);
        echo "</form></center></table><br><font color=#ff0000 size=3><b>ERROR $error: No se pudo ejecutar la consulta en la base de datos.</font><br>Descripci�n:<br>$msg</b>";
        return false;
    }
    if ($result)
        return $result;
...
function variables_form_busqueda($prefijo, $extra = array()) {
    ...
    $variables = array("cmd" => $cmd, "cmd1" => $cmd1, "page" => $page, "keyword" =>
        $keyword, "filter" => $filter, "sort" => $sort, "up" => $up);
...
function diferencia_dias_habiles($fecha1, $fecha2) {
    ...
    while (compara_fechas(fecha_db($fecha_aux), fecha_db($fecha2)) == -1) {
        //mientras la fecha2 sea mayor que la 1
...
function diferencia_dias($fecha1, $fecha2, $h = 0) {
    ...
    if ($h) {
        ...
        $minutos = date("i");
        $segundos = date("s");
        while (compara_fechas($fecha_aux, $fecha_hasta) == -1) {
            //mientras la fecha2 sea mayor que la 1
        ...
        while (compara_fechas(fecha_db($fecha_aux), $fecha_hasta) == -1) {
            //mientras la fecha2 sea mayor que la 1
            $fecha_split = split("/", $fecha_aux);
            $dif_dias++;
            $fecha_aux = date("d/m/Y", mktime(12, 0, 0, $fecha_split[1], $fecha_split[0] + 1, $fecha_split[2]));
...
function diferencia_dias_m($fecha1, $fecha2) {
    /* //defino fecha 1
      $fecha_split = split("/", $fecha1);
      $ano1 = $fecha_split[2];
      $mes1 = $fecha_split[1];
      $dia1 = $fecha_split[1];

      //defino fecha 2
      $fecha_split = split("/", $fecha2);
      $ano2 = $fecha_split[2];
      $mes2 = $fecha_split[1];
      $dia2 = $fecha_split[0];
    ...
    $fecha1db = Fecha_db($fecha1);
    $fecha2db = Fecha_db($fecha2);
    $fp = strtotime($fecha2db);
    $fn = strtotime($fecha1db);
    $nc = round(($fp - $fn) / 86400);

    return $nc; //$dias_diferencia;
}
...
function NumerosALetras($Numero) {
    ...
    while ($Numero != 0) {
        ...
        if (($Numero < 1000000000) and ($Numero >= 100000000)) {
            if ((Intval($Numero / 100000000) == 1) and (($Numero - (Intval($Numero /
                            100000000) * 100000000)) < 1000000)) {
                $letras .= (string) "cien millones ";
            } else {
                $letras = $letras & Centenas(Intval($Numero / 100000000));
                if ((Intval($Numero / 100000000) <> 1) and (Intval($Numero / 100000000) <> 5) and
                        (Intval($Numero / 100000000) <> 7) and (Intval($Numero / 100000000) <> 9)) {
        ...
        if (($Numero < 1000) and ($Numero > 99)) {
            if ((Intval($Numero / 100) == 1) and (($Numero - (Intval($Numero / 100) * 100)) <
                    1)) {
                //$letras = $letras & "cien ";
                $letras .= "cien";
            } else {
                ...
                $l2 = Centenas($temp);
                $letras .= (string) $l2;
                if ((Intval($Numero / 100) <> 1) and (Intval($Numero / 100) <> 5) and (Intval($Numero /
                                100) <> 7) and (Intval($Numero / 100) <> 9)) {
                    $letras .= "cientos ";
                } else {
                    $letras .= (string) " ";
                }
...
function generar_barra_nav($campos_barra) {
    ...
    foreach ($campos_barra as $clave => $valor) {
        ...
        if ($cmd == $valor["cmd"]) {
            ...
            $barra .= "<td id=$menuid class='bordesderinferior' width='$width%' style='cursor:pointer' onmouseover=\"this.style.color='#F7BE81'\" onmouseout=\"this.style.color='#2D3053'\" >";
            $barra .= '<a href="' . encode_link($_SERVER["PHP_SELF"], is_array($valor["extra"]) ?
                                    array_merge($valor["extra"], array("cmd" => $valor["cmd"])) : array("cmd" => $valor["cmd"])) .
                    '">';
            $barra .= $valor["descripcion"] . "</a></td>";
        } else {
...
function firma_mail() {
    ...
    $confiden .= "las personas direccionadas en el mail, puede contener informacion de ";
    $confiden .= "propiedad exclusiva de PLAN SUMAR del MSP Misiones y/o amparada por el secreto profesional.\n";
    $confiden .= "El acceso no autorizado, uso, reproduccion, o divulgacion esta prohibido.\n";
    return $confiden;
}

function encabezado_mail() {
    $confiden = "SISTEMA DE NOTIFICACIONES PLAN SUMAR - MSP MISIONES\n\n";
    return $confiden;
}
...
function enviar_mail($para, $paracc, $parabcc, $asunto, $contenido, $adjunto, $path, $htmlflag = 1) {
    $mail = new PHPMailer();
    $mail->Mailer = "smtp";
    $mail->Host = "smtp.arnetbiz.com.ar"; //servidor de mi trabajo
    $mail->Port = 25;
    $mail->SMTPAuth = true;
    $mail->Username = "areasistemasmsp@arnetbiz.com.ar";
    $mail->Password = "msalud2011"; //clave
    $mail->Timeout = 60; //segerido 50
    $mail->From = "areasistemasmsp@arnetbiz.com.ar";
    $mail->FromName = "Plan Sumar";
    $mail->AddAddress($para, $para);
    $mail->Subject = "SISTEMA DE NOTIFICACIONES - PLAN SUMAR - MSP MISIONES " . $asunto;
    $mail->Body = $contenido . "<BR> <BR>" . firma_mail();
    if (!$htmlflag)
        $mail->IsHTML(true);
    if ($paracc != '')
        $mail->AddCC($paracc);
    if ($parabcc != '')
        $mail->AddBCC($parabcc);
    if ($adjunto != '')
        $mail->AddAttachment($path . "/" . $adjunto, $adjunto);
    return $mail->Send();
}
...
function FileUpload($TempFile, $FileSize, $FileName, $FileType, $MaxSize, $Path, $ErrorFunction, $ExtsOk, $ForceFilename, $OverwriteOk, $comprimir = 1, $mostrar_carteles = 1) {
    ...
    if ($mostrar_carteles) {
        ...
        echo "<tr><td>Tama�o:</td><td>$FileSize</td></tr>\n";
        echo "<tr><td>Tipo MIME:</td><td>$FileType</td></tr>\n";
    }
    if ($TempFile == 'none' || $TempFile == '') {
        $ErrorTxt = "No se especific&oacute; el nombre del archivo<br>";
        $ErrorTxt .= "o el archivo excede el m&aacute;ximo de tama�o de:<br>";
    ...
    if ($FileSize == 0) {
        $ErrorTxt = 'El archivo que ha intentado subir, est&aacute; vac&iacute;o!';
        $retorno["error"] = 3;
        $ErrorFunction($retorno["error"], $ErrorTxt);
        return $retorno;
    }
     ...
     */
    if ($FileSize > $MaxSize) {
        $ErrorTxt = 'El archivo que ha intentado subir excede el m&aacute;ximo de ' . ($MaxSize /
                1024) . 'kb.';
        $retorno["error"] = 5;
        $ErrorFunction($retorno["error"], $ErrorTxt);
        return $retorno;
    ...
    $FileNameFullComp = substr($FileNameFull, 0, strlen($FileNameFull) - strpos(strrev
                                    ($FileNameFull), ".") - 1) . ".zip";
    ...
    if ($comprimir) {
        ...
        if ($mostrar_carteles)
            echo "<tr><td>Tama�o comprimido:</td><td>$FileSizeComp</td></tr>\n";
    }
...
function FileDownload($Comp, $FileName, $FileNameFull, $FileType, $FileSize, $zipguardado =
 1) {
...
function fin_pagina($debug = true, $mostrar_tiempo = true, $mostrar_consultas = true) {
    ...
    if ($mostrar_tiempo) {
        echo "Pagina generada en " . tiempo_de_carga() . " segundos.<br>";
    }
...
function permisos_check($modulo, $item) {
    ...
    if ($ouser->permisos[$item])
    // && ($ouser->permisos[$item]->dir=="" || $ouser->permisos[$item]->dir==$modulo))
        return true;
    else
        return false;
}
...
function generar_codigo_barra($barcode = '0123456789', $output = 'png', $width =
 '460', $height = '120', $xres = '2', $font = '5', $border = 'off', $drawtext =
 'off', $stretchtext = ' ', $negative = 'off', $redimweight = '', $redimheight =
 '') {
    global $html_root;
    //Genraci�n del C�digo de Barras
...
function codigo_barra($barcode = '0123456789', $output = 'png', $width = '460', $height = '120', $xres = '2', $font = '5', $border = 'off', $drawtext = 'off', $stretchtext =
 ' ', $negative = 'off', $redimweight = '', $redimheight = '') {
    global $html_root;
    //Genraci�n del C�digo de Barras
    if (isset($barcode) && strlen($barcode) > 0) {
        $style = BCS_ALIGN_CENTER;
        ...
        if ($obj) {
            if ($obj->DrawObject($xres)) {
                echo "<table align='center'><tr><td><img src='$html_root/lib/barcode/image.php?code=" .
                $barcode . "&style=" . $style . "&type=" . $type . "&width=" . $width .
                "&height=" . $height . "&xres=" . $xres . "&font=" . $font . "' $estilo></td></tr></table>";
            }
            else
...
function cargar_user($username) {
    ...
    $user = array("id" => $ouser->id_usuario, "login" => $ouser->login, "name" => $ouser->
        nombre . " " . $ouser->apellido, "mail" => $ouser->mail, "home" => $ouser->
        pagina_inicio, "res_width" => $_POST["resolucion_ancho"], "res_height" => $_POST["resolucion_largo"],
        "acceso1" => $ouser->accesos[0], "acceso2" => $ouser->accesos[1], "acceso3" => $ouser->
        accesos[2], "acceso4" => $ouser->accesos[3], "acceso5" => $ouser->accesos[4],
        "acceso6" => $ouser->accesos[5], "acceso7" => $ouser->accesos[6], "acceso8" => $ouser->
        accesos[7], "acceso9" => $ouser->accesos[8], "acceso10" => $ouser->accesos[9],
        "acceso11" => $ouser->accesos[10], "acceso12" => $ouser->accesos[11], "debug" =>
        "off" //		"permisos"	=> permisos_cargar($ouser)
    );
...
function actualizar_permisos_bd($id) {
    ...
    foreach ($array_usuario as $key => $id_usuario) {
        ...
        sql($query, "<br>Error al insertar/actualizar los permisos actualizados para el usuario<br>") or
                fin_pagina();
    }
}
...
function obtener_id_usuarios($id) {
    ...
    $sql = "select distinct id_usuario
		  from 
		  (select distinct id_usuario from permisos.permisos_usuarios where id_permiso in (" .
            join(",", $hijos) . ")
		  union
...
if (isset($_POST["loginform"])) {
    ...
    if ($status <= 0) {
        ...
        switch ($status) {
            case PHPSS_LOGIN_AUTHFAIL:
                Error("Su nombre de usuario o contrase&ntilde;a son incorrectos");
                break;
            case PHPSS_LOGIN_IPACCESS_DENY:
                Error("No se permite iniciar sesi�n desde su direcci&oacute;n IP");
                break;
            case PHPSS_LOGIN_BRUTEFORCE_LOCK_ACCOUNT:
                Error("Esta cuenta ha sido bloqueada debido a varios intentos fallidos de inicio de sesi&oacute;n");
                break;
            case PHPSS_LOGIN_BRUTEFORCE_LOCK_SRCIP:
                Error("No se puede iniciar sesi�n desde su direcci&oacute;n IP porque hubieron muchos intentos fallidos de inicio de sesi&oacute;n");
                break;
            default:
    ...
    while (!$result->EOF) {
        $feriados[$result->fields["anio"] . "-" . $result->fields["mes"] . "-" . $result->
                fields["dia"]][] = $result->fields["descripcion"];
        $result->MoveNext();
    }
...
// Tama�o m�ximo de los archivos a subir
...
$sino = array("0" => "NO", "f" => "NO", "false" => "NO", "NO" => "NO", "n" =>
    "NO", "1" => "SI", "t" => "SI", "true" => "SI", "SI" => "SI", "s" => "SI");
// Para el formato de fecha
$dia_semana = array("Domingo", "Lunes", "Martes", "Mi&eacute;rcoles", "Jueves",
...
if ($_ses_cambiar_perfil_usuario == 1 && !($parametros['cmd'] ==
        "cambiar_acceso" || $parametros['cmd'] == "actualizar_item" || $_POST["cmd"] ==
        "cambiar_acceso" || $_POST['cambiar_pagina_inicio'] || $_ses_pagina_inicio || $_ses_cambiar_acceso ||
...
if ($_ses_pagina_inicio) {
    ...
    $link = encode_link($html_root . "/modulos/admin/usuarios_perfil.php", array("pagina_item" =>
        $_SERVER["REQUEST_URI"], "cmd" => "actualizar_item"));
...
if ($_ses_cambiar_acceso) {
    ...
    //phpss_svars_set("_ses_cambiar_perfil_usuario", 1); //para que no actualize la ruta cuando cambia la pagina
    $link = encode_link($html_root . "/modulos/admin/usuarios_perfil.php", array("pagina_item" =>
        $_SERVER['REQUEST_URI'], "cmd" => "cambiar_acceso"));
    header("Location:$link");
    exit;
}
