 ...
 else
{
 ...
 this.timeoutops=setTimeout(new z.topMenu._delayedTimerCall(this,"onmouseover",0,0),this.defaultTimeout);
}
 ...
 for(var i=0;i<this.gitemsCount;i++)
{
 ...
 if(!node)
 {
	 node=that.xmlUnit.getXMLTopNode("tree");
	 level=0;
	 parentNode=that;
 ...
 if(level)
 {
	 var parentNode=new dhtmlXMenuBarPanelObject(parentNode,aTempNode,((mode!="classic")&&(mode!="popup")),node.getAttribute("panelWidth"),(that.xmlUnit.getXMLTopNode("tree").getAttribute("withoutImages")?1:(node.getAttribute("mixedImages")?2:0)));
	 //FILA ORIGINAL:
...
//		 }
		 ...
		 var z=eval("window.dhtmlXMenuItemObject");
	 	 var TempNode=null;
		 if(z)
		 ...
		 if(localItem.childNodes.length)
 ...
 if(id.tagName=="item")
{
 ...
 text=id.getAttribute("text");
 className=id.getAttribute("className");
 first_row=id.getAttribute("first_row");
 ...
 //this.className=className || "defaultMenuText";
 
 this.textclassName=first_row || "defaultMenuText";
 ...
 if((thatM.ieWinMode)&&(!thatM.ieWinModeState)&&(that.subMenu))
 {
	...
	if(thatM.realWinModeStart)
	{
	 	...
	 	//guarda el evento del body.onmouseup
	 	if(document.body.onmouseup!=thatM._onclickclose)
		{
	 	...
	 	//document.body.onclick=function (){alert('Frame1 say hello...');};
	
	 	//alert (document.getElementById('frame2').document.body);
	 	//document.getElementById('frame2').document.body.onclick=function (){alert('Frame2 say hello...')};
 ...
 if((that.parentPanel.topMenu.ieWinMode)&&(!that.parentPanel.parentPanel))
{
 ...
 setTimeout(new that.parentPanel.topMenu._delayedTimerCall(that,"_onclickY",0,0),10);
 return;
 ...
 if (parent.html_root != '' && this.href.indexOf(html_root) != 0)
	    this.href=parent.html_root+this.href;

	    //alert(this.href.indexOf(html_root));
	    form.action=this.href;
 form.target=this.target;
 form.method="POST";
 //for (i in this.parentPanel.parentItem.parentPanel.parentItem)
  //document.write(i+": "+this.parentPanel.parentItem.parentPanel.parentItem[i]+"<br><br>");
 writeRootPath(this);
 form.submit();
}
 else if (this.id=="calculadora")
	eval(this.href);
 else
 ...
 else
{
 ...
 if((!that.ieWinMode))
{
 this.timeoutop=setTimeout(new that._delayedTimerCall(this.panel.topMenu,"_onItemOut",0,this.panel),20);
}
};
 ...
 if(panel.topNod.ieFix)
{
 ...
 var uf=0;
 this.offsetWidthOld = panel.topNod.offsetWidth;
 if(panel.parentPanel){
 var xs=document.body.offsetWidth-5;
 //var xs=document.body.offsetWidth-15;
 var ys=document.body.offsetHeight-5;
 //document.body.offsetWidth,document.body.offsetHeight resolucion
// alert('width '+document.body.offsetWidth);
// alert('height '+document.body.offsetHeight);
// alert ('xs= '+xs);
// alert('topNod= ' + panel.topNod.offsetWidth);
// alert('topNodstyle = ' + parseInt(panel.topNod.style.left));
// alert ('suma = ' + (panel.topNod.offsetWidth+parseInt(panel.topNod.style.left)));
 //alert ('topNodwidth= ' +this.width);
 
 if((panel.topNod.offsetWidth+parseInt(panel.topNod.style.left))>xs)
{ //alert('EN IF');
 if(!panel.parentPanel.extraMode){
 var z=parseInt(panel.topNod.style.left) - panel.topNod.offsetWidth - panel.parentPanel.topNod.offsetWidth;
//alert(panel.parentPanel.topNod.offsetWidth);
if (z < 0) {
 z=xs - panel.topNod.offsetWidth;
// alert('z2='+ panel.parentPanel.topNod.offsetWidth);
}
 //var z=xs-panel.topNod.offsetWidth-parseInt(panel.topNod.style.left)+;
 //alert (xs);
...
//esta funcion escribe la ruta de acceso hasta el item en variables auxiliares
//para posterior uso de las mismas
function writeRootPath(item)
{ 
	//variable que se lee al cargar cualquier documento del frame2 y que pertenece a frame2.parent
	var pathString=item.text;
	
	var itemTemp=typeof item.parentPanel!='undefined'?item.parentPanel.parentItem:item.parentPanel;
	while (typeof itemTemp!='undefined')
	{
	  pathString=itemTemp.text +" > "+ pathString; //este es el item padre
	  itemTemp=itemTemp.parentPanel.parentItem;
	}
	if (parent.html_root != '' && item.href.indexOf(html_root) != 0)
	    parent.tempLinkHref=parent.html_root+item.href;
	else   
	    parent.tempLinkHref=item.href;
	    
	parent.tempLinkTarget=item.target;
	parent.tempLinkText=pathString;
}
;