...
To use this software please contact info@scbr.com to obtain license
*/ 
 ...
 if((this.globalNodeStorage[i].unParsed)&&(!skipParsing))
{
 ...
 if((itemId!=0)&&(!skipXMLSearch))return this.preParse(itemId);
 else return null;
};
 
 dhtmlXTreeObject.prototype.reParse=function(node){
 var that=this;
 if((this.onXLS)&&(!this.parsCount))window.setTimeout(function(){that.onXLS(that)},1);
 this.xmlstate=1;
...
{
 var zNodeId=this.findXML(z.childNodes[i],"id",itemId);
 if(zNodeId!=false)
{
 z=this._globalIdStorageFind(zNodeId);
...
dhtmlXTreeObject.prototype.setItemStyle=function(itemId,style_string){
 ...
 dhtmlXTreeObject.prototype._parseXMLTree=function(dhtmlObject,node,parentId,level,xml_obj){
 ...
 if((!dhtmlObject.slowParse)||(dhtmlObject.waitUpdateXML))
{
 ...
 window.setTimeout(function(){dhtmlObject._redrawFrom(dhtmlObject)},10);
}else{
 ...
 if((dhtmlObject.slowParse)&&(dhtmlObject.parsingOn==dhtmlObject.rootId))
{
 ...
 var nodelist=xml_obj.doXPath("//item[@open]");
 else{
 ...
 window.setTimeout(function(){dhtmlObject.onXLE(dhtmlObject)},1);
...
{img.style.width="0px";img.style.height="0px";}
 ...
 case "disabled": this.childCalc=0;break;
...
{
 if(this.lastSelected)
 if(this._globalIdStorageFind(this.lastSelected.parentObject.id,0,1))
 ...
 if(this.nodeCut)
{
 ...
 if(!temp)return 0;
 if(this.nodeCut){
 if((!this._checkParenNodes(this.nodeCut.parentObject.id,temp))&&(itemId!=this.nodeCut.parentObject.parentObject.id))
 this._moveNode(this.nodeCut.parentObject,temp);
 ...
 if(itemObject.unParsed)
{
 ...
 for(var i=0;i<itemObject.childNodes.length;i++)
 ...
 for(var i=0;i<temp.childsCount;i++)
{
 ...
 for(var i=0;i<node.childNodes.length;i++)
 if(node.childNodes[i].tagName=="item"){
 z+=this.dlmtr+node.childNodes[i].getAttribute("id");
 ...
 for(var i=0;i<Count;i++)
{
 ...
 if(sNode.dscheck)sNode.checkstate=3;
 ...
 if(this.dadmode==2)
{
 ...
 var z=y-a1+this.allTree.scrollTop-2-htmlNode.offsetHeight/2;
 ...
 if(!htmlObject.parentObject)
{
 ...
 if(shtmlObject.parentObject.childNodes==null){
...
{
 if((!this._onDrInFunc)||(this._onDrInFunc(shtmlObject.parentObject.id,htmlObject.parentObject.id)))
{
...
dhtmlXTreeObject.prototype._findNodeByLabel=function(searchStr,direction,fromNode){
 ...
 searchStr = new RegExp(searchStr.replace(/ /gi,"[^\n]*"),"gi");

 
 if(!fromNode)
{
 fromNode=this.lastSelected;
 if(!fromNode)fromNode=this.htmlNode;
 else fromNode=fromNode.parentObject;
}
