   ...
   while (!$result->EOF) {?>
    ...
    <tr <?=atrib_tr()?>>     
     ...
     <?}?>    
      ...
      ?>
      <td onclick="<?=$onclick_elegir1?>"><center><img src='../../imagenes/<?=$arch?>' style='cursor:pointer;' title="<?echo "Cantidad de Prestaciones: ".$result_tmp?>"> <?=$result_tmp?></center></td>  
      <?
      ...
      ?>
      <td onclick="<?=$onclick_elegir2?>"><center><img title="Cantidad Trazadora: <?=$res_1?>" src='../../imagenes/<?=$image?>' style='cursor:pointer;'> <?=$res_1?></center> </td>  
    </tr>    
    </tr>
	<?$result->MoveNext();
    }?>
