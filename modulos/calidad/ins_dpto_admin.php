  ...
  <table width=70% align="center" class="bordes">
        ...
        <tr>
           ...
           <td  colspan="2">
           	  ...
           	  $sql = "SELECT count (smiafiliados.id_smiafiliados)as r1 
					...
					left join facturacion.smiefectores ON (facturacion.smiefectores.cuie = nacer.smiafiliados.cuieefectorasignado)
     ...
     <tr>
       <td>
        <table>
        <?//tabla de comprobantes
		$query="select nombreefector,cuie, count (cuie) as cant_facturacion
				from facturacion.factura
				left join facturacion.smiefectores using (cuie)
				where departamento='$codigo' and estado='C'
				group by cuie,smiefectores.nombreefector
				order by nombreefector";
     ...
     <tr>
      <td id=mo colspan="2">
       <b>Nivel de Involucra.miento de los Prestadores</b>
      </td>
     </tr>
     <tr>
       <td>
       ...
       <?//tabla de comprobantes
		$query="select nombreefector,cuie, count (cuie) as cant_facturacion
				from facturacion.factura
				left join smiefectores.nombreefector using (cuie)
...
<?//tabla de comprobantes
$query="select smiefectores.nombreefector, count (id_smiafiliados) as total, departamento, cuie from 
facturacion.smiefectores
left join nacer.smiafiliados on  (facturacion.smiefectores.cuie=nacer.smiafiliados.cuieefectorasignado)
where departamento='$codigo' and activo='S'
group by nombreefector,departamento,cuie
order by nombreefector";
...
<tr><td><table width="100%" class="bordes" align="center">
	<tr align="center" id="mo">
	  <td align="center" width="3%">
	   <img id="imagen_2" src="<?=$img_ext?>" border=0 title="Mostrar" align="left" style="cursor:pointer;" onclick="muestra_tabla(document.all.prueba_vida1,2);" >
	  </td>
	  <td align="center">
	...
	<?if ($efectores->RecordCount()==0){?>
	 ...
	 <?}
	 	...
	 	while (!$efectores->EOF) {
	 		...
	 		<tr <?=atrib_tr()?>>	 				
		 		<td ><?=$i?></td>
		 		<td ><?=$efectores->fields['nombreefector']?></td>
	...
	<?if ($efectores_facturan->RecordCount()==0){?>
	 ...
	 <?}
	 	...
	 	while (!$efectores_facturan->EOF) {
	 		...
	 		<tr <?=atrib_tr()?>>	 				
		 		<td ><?=$i?></td>
		 		<td ><?=$efectores_facturan->fields['nombreefector']?></td>
...
<?//tabla de comprobantes
$query="select nombreefector, cuie, sum (monto_prefactura) as total, 'Total Prefacturado' as Detalle
from facturacion.factura
left join facturacion.smiefectores using (cuie)
where departamento='$codigo' and estado='C'
group by smiefectores.nombreefector,factura.cuie
union 
select nombreefector, factura.cuie, sum(cantidad*precio_prestacion) as total, 'Total Pagado' as Detalle
from facturacion.factura
left facturacion.smiefectores using (cuie)
left join facturacion.comprobante using (id_factura)
left join facturacion.prestacion using (id_comprobante)
where departamento='$codigo' and estado='C'
group by smiefectores.nombreefector,factura.cuie
union
select nombreefector, factura.cuie,sum(precio) as total_pagado, 'Total Pagado Nuevo Nomenclador' as Detalle
from nomenclador.prestaciones_n_op
left join facturacion.comprobante using (id_comprobante)
left join facturacion.factura using (id_factura)
left join facturacion.smiefectores using (cuie)
...
group by smiefectores.nombreefector,factura.cuie
union
select nombreefector, cuie, sum(cantidad*monto)as total, 'Total Debitado' as detalle
from facturacion.factura
left join facturacion.smiefectores using (cuie)
left join facturacion.debito using (id_factura)
where departamento='$codigo' and estado='C'
group by smiefectores.nombreefector,factura.cuie
union
select nombreefector,egreso.cuie,sum(monto_egreso) as total , 'Total Ejecutado' as detalle
from contabilidad.egreso
left join facturacion.smiefectores using (cuie)
where departamento='$codigo'
group by smiefectores.nombreefector,egreso.cuie
order by detalle,nombreefector";
