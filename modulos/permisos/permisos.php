	...
	<tr>
			...
			<table width="100%" height="100%">
				...
				<tr>
					...
					<b>Buscar:</b> <select id="select_usr_grp"  name="select_usr_grp"  onchange="if (this.value==-1) document.all.bbuscar2.disabled=1; else document.all.bbuscar2.disabled=0">
					...
					<div align="left" id='searchRes' style="height:80%;background-color:#f5f5f5;border :1px solid Silver;overflow:auto;" />
					</div>
<?
...
function AgregarItem() 
{
		...
		if (!chk_itemName(name.value))
		{
			alert("Ya existe un item con ese nombre '"+name.value+"'");
		...
		switch(nodeType)
		{
			//carpeta contenedora
			case 1:
				tree.insertNewItem(padre,id,desc.value,0,"folderOpen.gif","folderOpen.gif","folderClosed.gif");
			break;
			//pagina de menu
			case 2:
				tree.insertNewItem(padre,id,desc.value,0,"iexplore.jpg","iexplore.jpg","iexplore.jpg");
			break;
			...
			case 4:
				tree.insertNewItem(padre,id,desc.value,0,"item.gif","item.gif","item.gif");
//				tree.refreshItem(padre);
			break;
			//pagina fuera del menu
			case 3:
			default:
				tree.insertNewItem(padre,id,desc.value,0,"leaf.gif","leaf.gif","leaf.gif");
				type=3;
			break;
		...
		//		insertNewNext();
		tree.setUserData(id,"name",name.value);//nombre unico del item
		tree.setUserData(id,"nodeType",nodeType);//tipo de permiso
		tree.setUserData(id,"dir",path.value);//directorio (dentro de modulos) donde se ubica, en caso de ser una pagina
		last_id.value=id;		

		changes.add("add",id,padre,nodeType,name.value,desc.value,path.value);
		name.value = '';
		desc.value = '';
		path.value = '';
		parentId.value='';
