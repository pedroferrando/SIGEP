...
if ($_POST['guardar'] == "Actualizar Planilla") {
    ...
    $query = "UPDATE facturacion.detalles_perinatal
               SET cantidad='$dias_internacion',monto='$importe_dias_internacion'
             WHERE id_comprobante='$id_comprobante' and id_concepto='5'";

    sql($query, "Error al insertar la Planilla") or fin_pagina();

    actualizarPrestacion($id_comprobante, $importe_dias_internacion);
...
if ($_POST['guardar'] == "Guardar Planilla") {
    ...
    $id_prestacion = guardarPrestacion($id_comprobante, $id_nomenclador, '1', $importe_dias_internacion);
    ...
    $query = "INSERT INTO facturacion.detalles_perinatal
             ...
             VALUES
             ('$id_nomenclador','$clave_beneficiario','$importe_dias_internacion','$dias_internacion','5','$id_comprobante','$id_prestacion')";
...
if (!$resulta->EOF) {
    $estabacargado = true;
    $dias_internacion = $resulta->fields['cantidad'];
...
?>
<script>
    var concepto5 =<? echo $valoraciones_quirurjicas->fields['precio']; ?>;
    var maximo5 =<? echo $maximo5 = $valoraciones_quirurjicas->fields['maximo_dias'];
?>;

    $(document).ready(function() {
        $('#fecha_alta').datepicker({maxDate: "+0D"});
        calcularTotal();
        $('.form_prematurez').on('focusout', function() {
            if (($('#dias_internacion').val() > maximo5)) {
                alert('El valor ingresado supera el Maximo');
                $('#dias_internacion').val(0);
                $('#importe_dias_internacion').val(0);
                $('#importe_total').val(0);
            } else {
                calcularTotal();
            }
    ...
    function calcularTotal() {
        var dias_internacion = $('#dias_internacion').val() * concepto5;
        $('#importe_dias_internacion').val(dias_internacion);
    }
    ...
    function control_nuevos()
    {
        ...
        if ((document.all.importe_dias_internacion.value == "") || (document.all.importe_dias_internacion.value == "0")) {
...
<form name='form1' action='prematurez_admin.php' method='POST'>
    ...
    <table style="margin-top: 10px" width="85%" cellspacing=0 border=1 bordercolor=#E0E0E0 align="center" bgcolor='<?= $bgcolor_out ?>' class="bordes">
        <tr id="mo">
            <td>
                ...
                Prematurez<br> <?php echo "Prestacion: " . $codigo . " " . $diagnostico ?></td>
        </tr>
        <tr><td>
                    ...
                    <tr>
                        <td colspan='4'>
                            <table style="width:760px; ;margin: 0 auto 0 auto;padding-top: 5px;border:thin black solid">  
                                <tr>
                                    <td align="right">
                                        <b>Dias Internacion:</b>
                                    </td>         	
                                    <td align='left'>
                                        <input class="form_prematurez" type="text" size="8" value="<?php echo $dias_internacion ?>" id="dias_internacion" name="dias_internacion" maxlength="3" onKeyPress="return acceptNum(event)">(Maximo = <?= $maximo5 ?>)
                                    </td>
                                    <td align="right" style="padding-left: 10px">
                                        <b>Importe:</b>
                                    </td>         	
                                    <td align='left'>
                                        <input type="text" size="10" value="0" id="importe_dias_internacion" name="importe_dias_internacion" <? echo "readonly" ?>>
                                    </td>
                                </tr>
