...
<table cellspacing=2 cellpadding=2 border=0 width=100% align=center>
     <tr>
      <td align=center>
	    ...
	    &nbsp;&nbsp;<? $link=encode_link("nino_listado_new_excel.php",array());?>
        <img src="../../imagenes/excel.gif" style='cursor:pointer;'>
...
if ($_POST['generarnino']){
	$filename = 'N12200900000001.txt';	
    	...
    	//comienzo con los datos de trazadoras de otra tabla    	
    	$result3="SELECT * FROM trazadoras.nino
			left join facturacion.smiefectores using (CUIE)";
    	$result3=sql($result3) or die;
    	$result3->movefirst();
    	while (!$result3->EOF) {
			$contenido=$result3->fields['cuie'];
			$contenido.=chr(9);
    		$contenido.=chr(9);
    		$contenido.=$result3->fields['clase_doc'];
			$contenido.=chr(9);
    		$contenido.=$result3->fields['tipo_doc'];
			$contenido.=chr(9);
    		$contenido.=number_format($result3->fields['num_doc'],0,'','');
			$contenido.=chr(9);
    		$contenido.=trim($result3->fields['apellido']);
			$contenido.=chr(9);
    		$contenido.=trim($result3->fields['nombre']);
			$contenido.=chr(9);
    		$contenido.=$result3->fields['fecha_nac'];
			$contenido.=chr(9);
    		$contenido.=$result3->fields['fecha_control'];
			$contenido.=chr(9);
			if ($result3->fields['peso']=="0.000000")$peso="";
    		else $peso=number_format($result3->fields['peso'],3,".","");
			$contenido.=$peso;
			$contenido.=chr(9);			
    		if ($result3->fields['talla']=="0.000000")$talla="";
    		else $talla=number_format($result3->fields['talla'],0,"","");
			$contenido.=$talla;
			$contenido.=chr(9);
			if ($result3->fields['perim_cefalico']=="0.000000")$perim_cefalico="";
    		else $perim_cefalico=number_format($result3->fields['perim_cefalico'],3,".","");
			$contenido.=$perim_cefalico;
    		$contenido.=chr(9);
    		$contenido.=$result3->fields['percen_peso_edad'];
			$contenido.=chr(9);
    		if ($result3->fields['percen_talla_edad']=="-1")$percen_talla_edad="";
    		else $percen_talla_edad=$result3->fields['percen_talla_edad'];			
    		$contenido.=$percen_talla_edad;
			$contenido.=chr(9);
    		if ($result3->fields['percen_perim_cefali_edad']=="-1")$percen_perim_cefali_edad="";
    		else $percen_perim_cefali_edad=$result3->fields['percen_perim_cefali_edad'];
    		$contenido.=$percen_perim_cefali_edad;
			$contenido.=chr(9);
    		if ($result3->fields['percen_peso_talla']=="-1")$percen_peso_talla="";
    		else $percen_peso_talla=$result3->fields['percen_peso_talla'];
    		$contenido.=$percen_peso_talla;
			$contenido.=chr(9);
    		if ($result3->fields['triple_viral']!="1980-01-01") $contenido.=$result3->fields['triple_viral'];
			else $contenido.="";
			$contenido.="\n";
    		if (fwrite($handle, $contenido) === FALSE) {
        		echo "No se Puede escribir  ($filename)";
