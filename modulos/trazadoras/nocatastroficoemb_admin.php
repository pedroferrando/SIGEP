...
if ($_POST['guardar'] == "Actualizar Planilla") {
    ...
    $query = "UPDATE facturacion.detalles_perinatal
             ...
             WHERE id_comprobante='$id_comprobante' and id_concepto='6'";

    sql($query, "Error al insertar la Planilla") or fin_pagina();

    $query = "UPDATE facturacion.detalles_perinatal
               SET cantidad='$dias_comun',monto='$importe_dias_comun'
             WHERE id_comprobante='$id_comprobante' and id_concepto='7'";
...
if ($_POST['guardar'] == "Guardar Planilla") {
    ...
    $query = "INSERT INTO facturacion.detalles_perinatal
             ...
             VALUES
             ($id_nomenclador,'$clave_beneficiario',$importe_dias_uti,$dias_uti,12,$id_comprobante,$id_prestacion)";
    ...
    $query = "INSERT INTO facturacion.detalles_perinatal
             (id_nomenclador,clavebeneficiario,monto,cantidad,id_concepto,id_comprobante,id_prestacion)
             VALUES
             ($id_nomenclador,'$clave_beneficiario',$importe_dias_comun,$dias_comun,13,$id_comprobante,$id_prestacion)";
...
if (!$resulta->EOF) {
    ...
    $resulta->movenext();
    $dias_comun = $resulta->fields['cantidad'];
...
?>
<script>
    var concepto6 = <?= $valoraciones_quirurjicas->fields['precio'] ?>;
    var maximo6 =<?
echo $maximo6 = $valoraciones_quirurjicas->fields['maximo_dias'];
$valoraciones_quirurjicas->movenext();
?>;
    var concepto7 =<? echo $valoraciones_quirurjicas->fields['precio']; ?>;
    var maximo7 =<?
echo $maximo7 = $valoraciones_quirurjicas->fields['maximo_dias'];
?>;

    $(document).ready(function() {
        $('#fecha_alta').datepicker({maxDate: "+0D"});
        calcularTotal();
        $('.form_catastroficoemb').on('focusout', function() {
            if (($('#dias_uti').val() > maximo6) || ($('#dias_comun').val() > maximo7)) {
                alert('El valor ingresado supera el Maximo');
                if ($('#dias_uti').val() > maximo6) {
                    $('#dias_uti').val(0);
                    $('#importe_dias_uti').val(0);
                }
                if ($('#dias_comun').val() > maximo7) {
                    $('#dias_comun').val(0);
                    $('#importe_dias_comun').val(0);
                }
                $('#importe_total').val(0);
            } else {
                calcularTotal();
    ...
    function calcularTotal() {
        var dias_uti = $('#dias_uti').val() * concepto6;
        $('#importe_dias_uti').val(dias_uti);
        var dias_comun = $('#dias_comun').val() * concepto7;
        $('#importe_dias_comun').val(dias_comun);
        $('#importe_total').val(dias_uti + dias_comun);
    }
...
<form name='form1' action='nocatastroficoemb_admin.php' method='POST'>
    ...
    <table style="margin-top: 10px" width="85%" cellspacing=0 border=1 bordercolor=#E0E0E0 align="center" bgcolor='<?= $bgcolor_out ?>' class="bordes">
        <tr id="mo">
            <td>
                ...
                <? } ?>
                Embarazo de Alto Riesgo    <br> <?php echo "Prestacion: ".$codigo . " " . $diagnostico ?>
            </td>
        </tr>
        <tr><td>
                        ...
                        <td colspan='4'>
                            <table style="width:760px; ;margin: 0 auto 0 auto;padding-top: 5px;border:thin black solid">  
                                <tr>
                                    ...
                                    <td align='left'>
                                        <input class="form_catastroficoemb" type="text" size="8" value="<?php echo $dias_uti ?>" id="dias_uti" name="dias_uti" maxlength="2" onKeyPress="return acceptNum(event)">(Maximo = <?= $maximo6 ?>)
                                    </td>
                                ...
                                <tr>
                                    <td align="right">
                                        <b>Dias en Sala Comun:</b>
                                    </td>         	
                                    <td align='left'>
                                        <input class="form_catastroficoemb" type="text" size="8" value="<?php echo $dias_comun ?>" id="dias_comun" name="dias_comun" maxlength="3" onKeyPress="return acceptNum(event)">(Maximo = <?= $maximo7 ?>)
                                    </td>
                                    <td align="right" style="padding-left: 10px">
                                        <b>Importe:</b>
                                    </td>         	
                                    <td align='left'>
                                        <input type="text" size="10" value="0" id="importe_dias_comun" name="importe_dias_comun" <? echo "readonly" ?>>
                                    </td>
                                </tr>
                                <tr >
                                    ...
                                    <td align='left' colspan="2" style="padding-top: 10px">
                                        <input type="text" size="10" value="0" id="importe_total" name="importe_total" <? echo "readonly" ?>>
                                    </td>
                                </tr>
                            </table>
                        </td>
