...
$link = encode_link("resumen_promotor_excel.php", array("sql" => substr($sql, 0, strpos($sql, 'LIMIT'))));
...
if (isBusqueda()){
    ...
    if($promotores_filtroNombre == "NOTOSH"){
        ...
        $promotores_sql = "select p.apellido, p.nombre, p.dni, p.fechanac, l.nombre as localidad, f.cuie as efector, 
        ...
        group by p.apellido, p.nombre, p.dni, p.fechanac, l.nombre, f.cuie, 
        p.telefono, p.email, b.nombre, t.nombre, p.nrocuenta
    ...
    $rangoFecha_total = ($rangoFecha_fin - $rangoFecha_inicio)/(60 * 60 * 24);
}
...
<div name="mensaje" id="mensaje" align="center"><h1></h1></div>
...
<table border=0 width=100% cellspacing=2 cellpadding=2 bgcolor='<?=$bgcolor3?>' align=center>
 ...
 if (isBusqueda()){
    ...
    while(!$promotores->EOF){
        $editRef = encode_link("resumen_promotores_admin.php",array("idpromotor"=>$result->fields['idpromotor']));
        ...
        <tr >
            ...
            <td align=right bgcolor="<?=$bgTcolor?>"><?=$promotores->fields['fechanac']?></td>
            <td align=right bgcolor="<?=$bgTcolor?>"><?=($promotores->fields['telefono']." - ".$promotores->fields['email'])?></td>
            <td align=right bgcolor="<?=$bgTcolor?>"><?=$promotores->fields['efector']?></td>
            <td align=right bgcolor="<?=$bgTcolor?>"><?=($promotores->fields['localidad']."-")?></td>
