...
$titulo ='Cantidad de Fichas Cargadas';
$result ='';
...
if($_POST['buscar'] || $r!=''){
	...
	if($_POST['f_desde']!='' && $_POST['f_hasta']!=''){
		if($_POST['f_desde']!=$_POST['f_hasta']){
			$where_tmp.=" beneficiarios.fecha_carga between date('".Fecha_db($_POST['f_desde'])."') and (date('".Fecha_db($_POST['f_hasta'])."')+1) and";
			$titulo.=' desde '.$_POST['f_desde'].' hasta '.$_POST['f_hasta'];
		}else{
				$where_tmp.=" cast(fecha_carga as char(10)) = '".Fecha_db($_POST['f_desde'])."' and";
				$titulo.=' el '.$_POST['f_desde'];
			 }
	...
	if($_POST['c_hora_d']!='' && $_POST['c_hora_h']!='' && $_POST['c_min_d']!='' && $_POST['c_min_h']!=''){
		$where_tmp.=" (EXTRACT(HOUR FROM  fecha_carga)>".$_POST['c_hora_d']." or (EXTRACT(HOUR FROM  fecha_carga)=".$_POST['c_hora_d']." and EXTRACT(MINUTE FROM  fecha_carga)>=".$_POST['c_min_d'].")) AND (EXTRACT(HOUR FROM  fecha_carga)<".$_POST['c_hora_h']." or (EXTRACT(HOUR FROM  fecha_carga)=".$_POST['c_hora_h']." and EXTRACT(MINUTE FROM  fecha_carga)<=".$_POST['c_min_h'].")) and";
		$titulo.='; De '.$_POST['c_hora_d'].':'.$_POST['c_min_d'].' a '.$_POST['c_hora_h'].':'.$_POST['c_min_h'];
	}
	if($_POST['qbusk']!=''){
...
group by  cast(fecha_carga as char(10)) ,usuarios.nombre||' '||usuarios.apellido ";
	*/	
	$sql_tmp="select cast(fecha_carga as char(10)) as fecha_cargax,(usuarios.nombre||' '||usuarios.apellido) as nom_ape,count(*) as cant
					from uad.beneficiarios 
inner join  sistema.usuarios  on upper(beneficiarios.usuario_carga)=cast(usuarios.id_usuario as text) ";
	$sql_tmp.=$where_tmp;
	$sql_tmp.=" group by cast(fecha_carga as char(10)) ,usuarios.nombre||' '||usuarios.apellido 
				order by usuarios.nombre||' '||usuarios.apellido,cast(fecha_carga as char(10)) ";
	//sql($sql_tmp) or die; 	
...
<form name=form1 action="resumen_operador_hora2" method=POST>
<?echo "<center><b><font size='+1'>$titulo</font></b></center>";?>
<table cellspacing=2 cellpadding=2 border=0 width=100% align=center>
     <tr>
      <td align=center>
	    ...
	    &nbsp;&nbsp;<input type=submit name="buscar" value='Buscar'>		    
	     &nbsp;&nbsp;
	    <? $link=encode_link("resumen_operador_hora_excel2.php",array("sql"=>$sql_tmp)); ?>
	 ...
	 <b>Fecha Desde:</b><input type="text" size="10" maxlength="10" name="f_desde" onblur="esFechaValida(this);" onchange="esFechaValida(this);" onKeyUp="mascara(this,'/',patron,true);" value="<?=$_POST['f_desde']?>" <?=$r?>/><?=link_calendario('f_desde');?> <b>; Hasta: </b><input type="text" size="10" maxlength="10" name="f_hasta" onblur="esFechaValida(this);" onchange="esFechaValida(this);" onKeyUp="mascara(this,'/',patron,true);" value="<?=$_POST['f_hasta']?>" <?=$r?>/><?=link_calendario('f_hasta');} if($r=='' && $ad==''){?>&nbsp;&nbsp;&nbsp; <b>Grupo:</b>
	  ...
	  <tr>
	 <td align=center>
	 <b>De </b><select name="c_hora_d">
	 <option value="">?</option>
	 <? $i=0;
	 	while($i<=24){?>
 	 <option value="<?=str_pad($i,2,'0',STR_PAD_LEFT)?>"><?=str_pad($i,2,'0',STR_PAD_LEFT)?></option>
	 <? $i++;}?>
	 </select>:<select name="c_min_d">
	 <option value="">?</option>
	 <? $i=0;
	 	while($i<=59){?>
 	 <option value="<?=str_pad($i,2,'0',STR_PAD_LEFT)?>"><?=str_pad($i,2,'0',STR_PAD_LEFT)?></option>
	 ...
	 <b>; a </b><select name="c_hora_h">
	 <option value="">?</option>
	 <? $i=0;
	 	while($i<=24){?>
 	 <option value="<?=str_pad($i,2,'0',STR_PAD_LEFT)?>"><?=str_pad($i,2,'0',STR_PAD_LEFT)?></option>
	 <? $i++;}?>
	 </select>:<select name="c_min_h">
	 <option value="">?</option>
	 <? $i=0;
	 	while($i<=59){?>
 	 <option value="<?=str_pad($i,2,'0',STR_PAD_LEFT)?>"><?=str_pad($i,2,'0',STR_PAD_LEFT)?></option>
