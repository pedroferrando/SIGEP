...
FROM
  nacer.efe_conv
  inner join facturacion.smiefectores on (nacer.efe_conv.cuie =facturacion.smiefectores.cuie)";
...
if ($res_egreso->recordCount()==0){
	$sql="select ingre as total, ingre,egre,deve from
		...
		where cuie='$cuie') as deve";

}
else{
$sql="select ingre-egre as total, ingre,egre,deve from
		(select sum (monto_deposito)as ingre from contabilidad.ingreso
		where cuie='$cuie') as ingreso,
		(select sum (monto_egreso)as egre from contabilidad.egreso
		where cuie='$cuie') as egreso,
