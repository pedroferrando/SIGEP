...
if ($cmd=="Guardar") {	
	...
	elseif (!$nombre)
        ...
        elseif (!$uad)
		error ("Debe seleccionar el UAD del Usuario");
	elseif (!$apellido)
		 ...
		 //$nuevo_usuario=$login;
		  if ($db->Execute($sql)) {
			  ...
			  if (0){}
                                    ...
                                    $sql_sec_uad="SELECT nextval('uad.uad_x_usuario_id_uad_x_usuario_seq') as id_uad_x_usuario";
                                      $res_id_uas=sql($sql_sec_uad,"$sql_sec_uad") or fin_pagina();
                                      $id_uad_x_usuario=$res_id_uas->fields['id_uad_x_usuario'];

					$sql_sec_uad1="SELECT id_uad_x_usuario from uad.uad_x_usuario where id_usuario=$id_usuario";
                                      $res_id_uas1=sql($sql_sec_uad1,"$sql_sec_uad1") or fin_pagina();
                                      $id_uad_x_usuario1=$res_id_uas1->fields['id_uad_x_usuario'];
					if (!$id_uad_x_usuario1) {
                                    $sql_uad="insert into uad.uad_x_usuario
			                         (id_uad_x_usuario,cod_uad,id_usuario) values ($id_uad_x_usuario,'$uad',$id_usuario)";
			            sql($sql_uad,"$sql_uad") or fin_pagina();
					}
				   ...
				   if ($db->Execute($sql)) {
			          ...
			          if ($id_grupo) {
					   ...
					   $sql="DELETE FROM uad.uad_x_usuario WHERE id_usuario='$id_usuario'";
					   $db->Execute($sql);
  ...
  <td>
   <table width=100% border=0  bgcolor="<?=$bgcolor3?>">
	...
	<tr>
	 ...
	 <td>
                <select name=uad Style="width=300px">
			 <!--<option value=-1>Seleccione</option>-->
			 <?
			 $sql= "select * from uad.parametros order by parametros.codigo_uad";
			 $res_efectores=sql($sql) or fin_pagina();
			 while (!$res_efectores->EOF){
                         ?>
                            <option value='<?=$res_efectores->fields['codigo_uad']?>' 
                                    <? if ($res_efectores->fields['codigo_uad'] == '700') echo "selected" ?> >
                                    <?=$res_efectores->fields['codigo_uad'].'-'.$res_efectores->fields['nombre_uad']?>
                            </option>
			    <?
