...
$id_usuario_conect=$_ses_user['id'];
if ($cmd == "") {
	$cmd="conectados";
...
function conectados() {
	global $session_timeout,$bgcolor2,$bgcolor3,$up,$itemspp,$cmd,$id_usuario_conect;
	$orden = array(
		"default_up" => "1",
		"default" => "2",
		"1" => "usuarios.login",
	...
	$where_tmp = "(phpss_session.active='true' 
	              AND extract(year from lastrequest) = extract(year from CURRENT_DATE) 
				  AND extract(day from lastrequest) = extract(day from CURRENT_DATE) 
				  AND extract(month from lastrequest) = extract(month from CURRENT_DATE))";// lastrequest > '$fecha_limite')";
	...
	$where_tmp.= "and usuarios.id_usuario in (select id_usuario 
														FROM permisos.grupos
														left join permisos.grupos_usuarios on grupos.id_grupo=grupos_usuarios.id_grupo  
...
function historial() {
	...
	$sql_tmp = "SELECT phpss_session.id,ip,created,lastrequest,login,
		...
		LEFT JOIN permisos.phpss_ip ON phpss_ip.id = phpss_session.ipfid
		LEFT JOIN sistema.usuarios ON usuarios.login = phpss_account.username";
	$where_tmp = "(phpss_session.active='false' and lastrequest <= '$fecha_limite')";
