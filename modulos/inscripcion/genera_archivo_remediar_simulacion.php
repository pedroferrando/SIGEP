...
,beneficiarios.observaciones,remediar_x_beneficiario.fechaempadronamiento as fecha_inscripcion,remediar_x_beneficiario.fecha_carga,remediar_x_beneficiario.usuario_carga
...
,MAX(remediar_x_beneficiario.fechaempadronamiento)fechaempadronamiento, rf.nroformulario as nroformulario
			...
			inner join uad.remediar_x_beneficiario on remediar_x_beneficiario.clavebeneficiario=beneficiarios.clave_beneficiario 
                        left join remediar.formulario rf on remediar_x_beneficiario.nroformulario = rf.nroformulario";


$sql_tmp2.="                    WHERE length(beneficiarios.numero_doc) in (6,7,8) ";
//$sql_tmp2.="                  AND beneficiarios.estado_envio in ('e','n') ";
$sql_tmp2.=" 			AND UPPER(beneficiarios.estado_envio) in ('E','N','X') ";
$sql_tmp2.=" 			AND not exists (select id_beneficiarios from remediar.listado_enviados rle where rle.id_beneficiarios = beneficiarios.id_beneficiarios) ";
$sql_tmp2.=" 			AND NOT EXISTS(SELECT tipo_doc, documento FROM puco.puco WHERE tipo_doc = tipo_documento AND documento = CAST(numero_doc AS BIGINT)) ";
$sql_tmp2.=" 			AND remediar_x_beneficiario.fechaempadronamiento <= '$fechaemp' ";

$sql_tmp2.=" 			AND remediar_x_beneficiario.fecha_carga <= '$fechakrga' ";
...
$sql_tmp2.=" 			AND UPPER(beneficiarios.fallecido) = 'N' ";
...
,beneficiarios.observaciones,remediar_x_beneficiario.fechaempadronamiento,remediar_x_beneficiario.fecha_carga,remediar_x_beneficiario.usuario_carga
,beneficiarios.activo,beneficiarios.score_riesgo,beneficiarios.mail,beneficiarios.celular,beneficiarios.otrotel,beneficiarios.estadoest,beneficiarios.fum
,beneficiarios.obsgenerales,beneficiarios.estadoest_madre,beneficiarios.tipo_ficha,beneficiarios.responsable,beneficiarios.discv,beneficiarios.disca
,beneficiarios.discmo,beneficiarios.discme,beneficiarios.otradisc,beneficiarios.estadoest_padre,beneficiarios.estadoest_tutor,beneficiarios.menor_embarazada
,beneficiarios.apellido_benef_otro,beneficiarios.nombre_benef_otro,beneficiarios.fecha_verificado,beneficiarios.usuario_verificado,beneficiarios.apellidoagente
,beneficiarios.nombreagente,beneficiarios.centro_inscriptor,beneficiarios.dni_agente,smiefectores.nombreefector,relacioncodigos.codremediar, rf.nroformulario";
...
/////HEADER
    	...
    	while (!$result12->EOF) {
                        ...
                        if ($result12->fields['nroformulario'] == "0") {
                            $result12->fields['score_riesgo'] = '';
                        }
			$where2.=',';
			$where2_2.=',';
...
";
		...
		if (fwrite($handle2, $contenido2) === FALSE) {
			...
			$where2_2=substr($where2_2,1,strlen($where2_2));  
                            echo "El Archivo ($filename_remediar) se genero con exito"; 
                            echo "<a href='../remediar/$filename_remediar'>Ver</a>";
