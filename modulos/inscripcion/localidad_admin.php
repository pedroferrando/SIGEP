...
if ($borra_loca=='borra_loca'){
	...
	$query="delete * from uad.localidades
			where uad.localidades.id_localidad=$id_localidad";
...
<tr><td><table width="100%" class="bordes" align="center">
	...
	<tr><td><table id="prueba_vida" border="1" width="100%" style="display:none;border:thin groove">
			...
			if(provincia_uso()=='MISIONES'){$nomcampoloc='idloc_provincial'; $nomcampomun='idmuni_provincial'; }else{ $nomcampoloc='id_localidad'; $nomcampomun='idmuni_provincial'; } 
			 $query="select * ,$nomcampoloc, uad.localidades.nombre as nom_localidad
					 from  uad.localidades
					 INNER JOIN uad.codpost ON (uad.localidades.$nomcampoloc =uad.codpost.id_localidad)
					WHERE
					  id_departamento = '$id_departamento'";
			...
			if ($res_comprobante->RecordCount()==0){?>
				 ...
				 <?}
				 	...
				 	while (!$res_comprobante->EOF) {
				 		   		...
				 		   		$ref = encode_link("nuevo_muni.php",array("id_localidad"=>$res_comprobante->fields[''.$nomcampoloc.''],"nom_localidad"=>$res_comprobante->fields['nom_localidad'],"id_departamento"=>$res_comprobante->fields['id_departamento']));
			            ...
			            $id_tabla="tabla_".$res_comprobante->fields['id_localidad'];	
				 		...
				 		<tr <?=atrib_tr()?>>
					 		...
					 		<? $ref=encode_link("localidad_admin.php",array("id_localidad"=>"'".$res_comprobante->fields[''.$nomcampoloc.'']."'","borra_loca"=>"borra_loca", "id_departamento"=>$res_comprobante->fields['id_departamento'])); 
					 		$onclick_localidad="if (confirm('Seguro que desea eliminar la Localidad?')) location.href='$ref'"; ?>
					 		<td align="center"><img src='../../imagenes/salir.gif' style='cursor:pointer;' onclick="<?=$onclick_localidad?>"></td>
					 	</tr>
					 <?	//---------------------municipio------------------------------	?>
					 	<tr>
				          ...
				          <td colspan=6>
				           <? $id_localidad=$res_comprobante->fields['id_localidad'];
				                $sql=" select * from  uad.municipios													
										WHERE $nomcampomun = '$id_localidad'";										
				                 	...
				                 	<table width=90% align=center border="1" class=bordes>
										    ...
										    <td align=right id=mo colspan="2">Borrar</a></td> 
					                            <? while (!$result_items->EOF){
					                            	...
					                            	$ref = encode_link("nuevo_barrio.php",array("id_municipio"=>$result_items->fields[''.$nomcampomun.''],"nom_municipio"=>$result_items->fields['nombre']));
    												...
    												<td onclick="<?=$onclick_elegir?>"><?=$result_items->fields["nombre"]?></td>
						                            	 ...
						                            	 <? $ref=encode_link("localidad_admin.php",array("id_municipio"=>$result_items->fields[''.$nomcampomun.''],"borra_muni"=>"borra_muni","id_localidad"=>$result_items->fields[''.$nomcampomun.''],"id_departamento"=>$id_departamento )); 
												 		$onclick_muni="if (confirm('Seguro que desea eliminar el Municipio?')) location.href='$ref'"; ?>
												 	<td align="center"><img src='../../imagenes/salir.gif' style='cursor:pointer;' onclick="<?=$onclick_muni?>"></td>
