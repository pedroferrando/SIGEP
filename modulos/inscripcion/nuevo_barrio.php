...
cargar_calendario();
if(provincia_uso()=='MISIONES'){$relacion_tablas=' on localidades.idloc_provincial=municipios.idmuni_provincial'; $nomcampomun='idmuni_provincial'; }else{ $relacion_tablas='using (id_localidad)'; $nomcampomun='idmuni_provincial'; }
...
if ($id_municipio){
	...
	$query="select * from uad.municipios  
			where $nomcampomun=$id_municipio";
...
<tr><td><table width=100% align="center" class="bordes">
  <tr align="center">
   <td>
   	...
   	<? $sql01="SELECT id_pais,trim(departamentos.nombre) as nombre,departamentos.id_departamento
            from uad.provincias 
	  		inner join uad.departamentos using (id_provincia)
	  		inner join uad.localidades using (id_departamento)
	  		inner join uad.municipios $relacion_tablas
	  		where $nomcampomun=$id_municipio";
				 	...
				 	while (!$res_comprobante->EOF) {
			            ...
			            $id_tabla="tabla_".$res_comprobante->fields['id_barrio'];	
				 		...
				 		<tr <?=atrib_tr()?>>
					 		...
					 		<td align="center"><img src='../../imagenes/salir.gif' style='cursor:pointer;' onclick="<?=$onclick_barrio?>"></td>
					 	</tr>
