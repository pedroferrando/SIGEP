...
$beneficiarios_sql = "select s.idseguimiento, s.clavebeneficiario, s.dmta, s.hta, s.tasist, s.tadias, s.tabaquismo, s.colesterol, s.glucemia, s.peso, s.talla, s.hba1c, s.ecg, 
...
$fp = fopen($archivoSeguimiento->getProtoripoArchivo(),"w");
fwrite($fp, $archivoSeguimiento->getLineasArchivo());
fclose($fp);

echo "<div align='center'><br><h2>El Archivo (".$archivoSeguimiento->getProtoripoArchivo().") se genero con exito </h2>";
echo "<a href='".$archivoSeguimiento->getProtoripoArchivo()."'><h2>Ver</h2></a></div>";
