...
$sql_tmp1 = "SELECT distinct beneficiarios.estado_envio,trim(clasificacion_remediar.dbt) as diabetes,beneficiarios.clave_beneficiario,beneficiarios.apellido_benef
...
$sql_tmp1.=" 			AND upper(beneficiarios.fallecido) ='N' ";
...
';
    ...
    while (!$result12->EOF) {
        ...
        #   NOTA: El desarrollador anterior del sistema confundió DMT y lo puso como
        #   DBT, en el cual se dettalla el tipo de diabetes, 1,2 o 0 si no tiene diabetes.
        #   El campo dmt2 está sin uso. Para corregir esto, se realiza la siguiente modificacion.
        
        
        if ($result12->fields['diabetes'] == "0" | $result12->fields['diabetes'] == '') {
            $contenido2.='""';   // 1       DMT    DATOS N34
        }else{
            
            $contenido2.='"DMT' . $result12->fields['diabetes'] . '"';   // 1       DBT    DATOS N34
        }
        $contenido2.=';';
...
$sql_tmp2 = "SELECT distinct beneficiarios.estado_envio,trim(clasificacion_remediar2.dmt) as diabetes,beneficiarios.clave_beneficiario,beneficiarios.apellido_benef
,beneficiarios.nombre_benef,beneficiarios.clase_documento_benef,beneficiarios.tipo_documento,beneficiarios.numero_doc,beneficiarios.sexo
,beneficiarios.fecha_nacimiento_benef,beneficiarios.provincia_nac,beneficiarios.localidad_nac,beneficiarios.pais_nac
,beneficiarios.cuie_ea,beneficiarios.cuie_ah,beneficiarios.calle
,beneficiarios.numero_calle
...
$sql_tmp2.=" 			AND UPPER(trim(clasificacion_remediar2.enviado)) = 'N'";
...
';
    ...
    while (!$result13->EOF) {
        ...
        if ($result13->fields['diabetes'] == "0" | $result13->fields['diabetes'] == '') {
            $contenido2.='""';   // 1       DMT    DATOS N34
        }else{
            
            $contenido2.='"DMT' . $result13->fields['diabetes'] . '"';   // 1       DBT    DATOS N34
        }
        $contenido2.=';';
