...
if ($_POST['nomenclador_detalle']){
	//cambia el nomenclador asignado al convenio
        //automáticamente al cambiar en el combo
...
if ($_POST['guardar_obl']=="Guardar Datos Obligatorios"){
	...
	$query="UPDATE facturacion.prestacion 
		SET tension_arterial='$tension_arterial', 
                    peso='$peso'
   ...
   if (($pagina_viene=='comprobante_admin_total.php')||((valida_prestacion($id_comprobante,$nomenclador))&&(valida_prestacion1($id_comprobante,$nomenclador)))){
	   $db->StartTrans();
	    ...
	    $query="INSERT INTO facturacion.prestacion
	            (id_prestacion,id_comprobante, id_nomenclador,cantidad,precio_prestacion,id_anexo,peso,tension_arterial)
	            VALUES ($id_prestacion,$id_comprobante,$nomenclador,$cantidad,$precio_prestacion,$anexo,'0','00/00')";
...
/*busca el nomenclador vigente asociado al convenio */
$sql="SELECT *
      FROM nacer.efe_conv
...
//si es modo facturacion 2 (nuevo nomenclador) llama a otro formulario de carga de prestacion
//se toma de nomenclador_detalle (nomenclador asociado al convenio)
if ((($modo_facturacion==2)&&($pagina_viene=='comprobante_admin_total.php'))||$entidad_alta=='in') {
	$ref = encode_link("prestacion_admin_2011.php",array("id"=>$id,"id_smiafiliados"=>$id_smiafiliados,"id_comprobante"=>$id_comprobante,"estado"=>"","pagina_listado"=>$pagina_listado,"pagina_viene"=>"comprobante_admin_total.php","entidad_alta"=>$entidad_alta));
...
function cambiar_nomenclador(){
	borrar_buffer(); 
	if (confirm ('Esta Accion Cambiara el Nomenclador Asociado al Efector: <?=$nombreefector;?>. ?Esta Seguro?')){
     ...
     <td>
         <?if ($pagina_viene!='comprobante_admin_total.php'){?>
          <table width=70% align="center" class="bordes">
             <tr>
              <td id=mo colspan="2">
                  <b> Descripci&oacute;n del COMPROBANTE</b>
              </td>
             </tr>
             <tr>
               <td>
                ...
                <table>
                    ...
                    <tr>
                        ...
                        <td align='left'>
                            <input type='text' name='afiapellido' value='<?=$nombreefector;?>' size=60 align='right' readonly></b>
                        </td>
                    </tr>         
                    <tr>
                       <td align="right">
         ...
         <!--habilita carga de una Nueva Prestacion -->
	 <table class="bordes" align="center" width="70%">
	    ...
	    <tr>
			 ...
			 <tr>
				    ...
				    <td align="left">
						...
						$grupo_nomenclador=utf8_decode('NIÑO');
					?>		          			
				 	<select name=nomenclador Style="width:700px"
				 		...
				 		onchange="borrar_buffer(); document.forms[0].submit();">
                                             ...
                                             if ($res_flag->recordCount()>0){
                                                 //solo carga los que estaban en facturacion.efec_nom
                                                 $sql= "SELECT * 
                                                        FROM facturacion.nomenclador n
                                                        INNER JOIN facturacion.efec_nom e ON trim(n.codigo)=trim(e.codigo)
                                                        WHERE cuie='$cuie' 
                                                            AND id_nomenclador_detalle='$id_nomenclador_detalle' 
                                                            AND n.codigo<>'DIFERENCIA DE NOMENCLADOR' 
                                                            AND n.codigo<>'DEB-CRED' 
                                                            AND n.grupo<>'$grupo_nomenclador'
                                                        ORDER BY n.codigo";
                                             } else {
                                                 //carga todo lo del nomenclador
                                                    $sql= "SELECT * 
                                                           FROM facturacion.nomenclador 
                                                           WHERE id_nomenclador_detalle='$id_nomenclador_detalle' 
                                                                 and nomenclador.codigo<>'DIFERENCIA DE NOMENCLADOR' 
                                                                 and nomenclador.codigo<>'DEB-CRED' 
                                                                 and nomenclador.grupo<>'$grupo_nomenclador' 
                                                           ORDER BY codigo";	
                                             }
                                            ...
                                            onchange="borrar_buffer();">			     			
                                             ...
                                             if ($nomenclador<>'')
                                                    $sql= "SELECT * 
                                                           ...
                                                           WHERE id_nomenclador_detalle='$id_nomenclador_detalle' 
                                                                AND id_nomenclador='$nomenclador'
	    ...
	    <tr>
	  	<td align="center" colspan="2" class="bordes">
			<?		  		
	  		if (es_cuie($_ses_user['login'])&&(($res_codigo=='MEM 01')||($res_codigo=='MER 15')||($res_codigo=='MER 16')||($res_codigo=='NPE 32')||($res_codigo=='NPE 33'))) 
            ...
            <tr align="center">
                <td>
                    ...
                    if ($pagina_viene=='comprobante_admin_total.php'){
                     ...
                     <?}
                     switch ($res_codigo){
                        case 'MEM 01' : $ref = encode_link("../trazadoras/emb_admin.php",array("fecha_comprobante"=>$fecha_comprobante,"id_smiafiliados"=>$id_smiafiliados,"cuie"=>$cuie,"pagina"=>"prestacion_admin.php"));
                                break;

                                case 'REM 76' : $ref = encode_link("../trazadoras/emb_admin.php",array("fecha_comprobante"=>$fecha_comprobante,"id_smiafiliados"=>$id_smiafiliados,"cuie"=>$cuie,"pagina"=>"prestacion_admin.php"));
                                break;

                        case 'MPA 17': $ref = encode_link("../trazadoras/par_admin.php",array("fecha_comprobante"=>$fecha_comprobante,"id_smiafiliados"=>$id_smiafiliados,"cuie"=>$cuie,"pagina"=>"prestacion_admin.php"));
                                break;

                        case 'MPA 18': $ref = encode_link("../trazadoras/par_admin.php",array("fecha_comprobante"=>$fecha_comprobante,"id_smiafiliados"=>$id_smiafiliados,"cuie"=>$cuie,"pagina"=>"prestacion_admin.php"));
                                break;

                        case 'NPE 32' :  if (date("Y-m-d")>='2011-01-01') $link_trz_nino='nino_admin_new.php';
                                                        else $link_trz_nino='nino_admin.php';
                                                        $ref = encode_link("../trazadoras/$link_trz_nino",array("fecha_comprobante"=>$fecha_comprobante,"id_smiafiliados"=>$id_smiafiliados,"cuie"=>$cuie,"pagina"=>"prestacion_admin.php"));
                                break;
                     ...
                     switch ($res_codigo){
                        case 'MEM 01' : ?>&nbsp;&nbsp;&nbsp;<input type=button name="carga_trazadora" value="Trazadora Embarazada" onclick="document.all.guardar.disabled=false; window.open('<?=$ref?>','Trazadoras','dependent:yes,width=900,height=700,top=1,left=60,scrollbars=yes');" title="Carga Trazadora Embarazada" style="width:150px;background-color:#F781F3;"> <?
                                break;
                                case 'REM 76' : ?>&nbsp;&nbsp;&nbsp;<input type=button name="carga_trazadora" value="Trazadora Embarazada" onclick="document.all.guardar.disabled=false; window.open('<?=$ref?>','Trazadoras','dependent:yes,width=900,height=700,top=1,left=60,scrollbars=yes');" title="Carga Trazadora Embarazada" style="width:150px;background-color:#F781F3;"> <?
                                break;			
                        case 'MPA 17': ?>&nbsp;&nbsp;&nbsp;<input type=button name="carga_trazadora" value="Trazadora Partos" onclick="document.all.guardar.disabled=false; window.open('<?=$ref?>','Trazadoras','dependent:yes,width=900,height=700,top=1,left=60,scrollbars=yes');" title="Carga Trazadora Partos" style="width:150px;background-color:#F781F3;"> <?
                                break;
                        case 'MPA 18': ?>&nbsp;&nbsp;&nbsp;<input type=button name="carga_trazadora" value="Trazadora Partos" onclick="document.all.guardar.disabled=false; window.open('<?=$ref?>','Trazadoras','dependent:yes,width=900,height=700,top=1,left=60,scrollbars=yes');" title="Carga Trazadora Partos" style="width:150px;background-color:#F781F3;"> <?
                                break;
                        case 'NPE 32': ?>&nbsp;&nbsp;&nbsp;<input type=button name="carga_trazadora" value="Trazadora Niño" onclick="document.all.guardar.disabled=false; window.open('<?=$ref?>','Trazadoras','dependent:yes,width=900,height=700,top=1,left=60,scrollbars=yes');" title="Carga Trazadora Niño" style="width:150px;background-color:#F781F3;"> <?
                                break;
                                case 'RPE 86': ?>&nbsp;&nbsp;&nbsp;<input type=button name="carga_trazadora" value="Trazadora Niño" onclick="document.all.guardar.disabled=false; window.open('<?=$ref?>','Trazadoras','dependent:yes,width=900,height=700,top=1,left=60,scrollbars=yes');" title="Carga Trazadora Niño" style="width:150px;background-color:#F781F3;"> <?
                                break;
                        case 'NPE 33': ?>&nbsp;&nbsp;&nbsp;<input type=button name="carga_trazadora" value="Trazadora Niño" onclick="document.all.guardar.disabled=false; window.open('<?=$ref?>','Trazadoras','dependent:yes,width=900,height=700,top=1,left=60,scrollbars=yes');" title="Carga Trazadora Niño" style="width:150px;background-color:#F781F3;"> <?
                                break; 
                                case 'RPE 87': ?>&nbsp;&nbsp;&nbsp;<input type=button name="carga_trazadora" value="Trazadora Ni?o" onclick="document.all.guardar.disabled=false; window.open('<?=$ref?>','Trazadoras','dependent:yes,width=900,height=700,top=1,left=60,scrollbars=yes');" title="Carga Trazadora Ni?o" style="width:150px;background-color:#F781F3;"> <?
                                break;			
                     }?>
