...
if ($entidad_alta=='nu' || $entidad_alta=='in'){//carga de prestacion a paciente NO PLAN NACER
	$link=encode_link("comprobante_admin_total.php", array("entidad_alta"=>$entidad_alta,"id"=>$id_smiafiliados,"pagina_viene"=>"comprobante_admin.php","pagina_listado"=>$pagina_listado));?>
...
if (($_POST['guardar']=="Guardar Comprobante")||($_POST['guardar']=="Guardar Comprobante y Facturar")){
	...
	if ($pagina_listado=='listado_beneficiarios_hist.php') {
		...
		if ($res_parametro=='por_periodo')$sql_hist="select count(id_smiafiliados) as cant from nacer.historicotemp
											where clavebeneficiario = '$clavebeneficiario' and periodo = '$periodo_comprobante_hist'";
		if ($res_parametro=='por_fecha')$sql_hist="select count(id_smiafiliados) as cant from nacer.historicotemp
											where clavebeneficiario = '$clavebeneficiario' and periodo = '$fecha_comprobante_hist'";
	    ...
	    else 
	    {
	      ...
	      $db->StartTrans();
		    ...
		    $query="insert into facturacion.comprobante
		             ...
		             values
		             ($id_comprobante,'$cuie','$nom_medico','$fecha_comprobante','$clavebeneficiario', $id_smiafiliados,'$fecha_carga','$periodo','$comentario','$servicio','$activo')";	
		    ...
		    if ($_POST['guardar']=="Guardar Comprobante y Facturar"){
		    	$ref = encode_link("prestacion_admin.php",array("id_smiafiliados"=>$id_smiafiliados,"id_comprobante"=>$id_comprobante,"estado"=>"","pagina_listado"=>$pagina_listado,"pagina_viene"=>"comprobante_admin.php"));
        ...
        <table>
         ...
         <tr>
          ...
          <tr>
           ...
           <td colspan="2">
         	 <input type='text' name='afidni' value='<?=date("Y-m-d")-$afifechanac?>' size=60 align='right' readonly></b>
           </td>
          </tr>
          ...
          <tr>
           ...
           <td colspan="2">
            <input type='text' name='descripcion' value='<?=$descripcion;?>' size=60 align='right' readonly></b>
           </td>
          </tr>
          <tr>
           <td align="right">
           ...
           <td colspan="2">
             <input type='text' name='nombreefecto' value='<?=$nombreefector;?>' size=60 align='right' readonly></b>
           </td>
          </tr>
        </table>
      </td>      
		 ...
		 <tr><td class="bordes"><table>
			 <tr>
				 <td>
					 <tr>
					    ...
					    <td align="left">		          			
			                 ...
			                 if (es_cuie($_ses_user['login']))
			                 	$sql= "select n.cuie, nombreefector, upper(trim(com_gestion))as com_gestion from nacer.efe_conv n inner join facturacion.smiefectores s on n.cuie=s.cuie where cuie='$user_login1' order by nombreefector";	
			                 else{
					 ...
					 <tr>
         	<td align="right">
                    <b>Periodo de Facturaci&oacute;n:</b>
			</td>
			<td align="left">		          			
	 ...
	 <?}
	 	...
	 	while (!$res_comprobante->EOF) {
	 		if ($res_comprobante->fields['id_factura']==""){
	 			$color_fondo="#FFFFCC"; 		
	 			$ref = encode_link("prestacion_admin.php",array("id_smiafiliados"=>$id_smiafiliados,"id_comprobante"=>$res_comprobante->fields['id_comprobante'],"pagina_listado"=>$pagina_listado,"pagina_viene"=>"comprobante_admin.php","estado"=>$res_comprobante->fields['id_factura']));
            	$onclick_elegir="location.href='$ref'";  
            	if ($res_comprobante->fields['marca']==0){
            		$ref1 = encode_link("comprobante_admin.php",array("id_comprobante"=>$res_comprobante->fields['id_comprobante'],"marcar"=>"True","id_smiafiliados"=>$id_smiafiliados));
...
<br>
	<table align='center' border=1 bordercolor='#000000' bgcolor='#FFFFFF' width='80%' cellspacing=0 cellpadding=0>
     <tr>
         <td colspan=10 bordercolor='#FFFFFF'><b>Colores de Referencia para la Columna N&uacute;mero de Comprobante:</b></td>
     <tr>
