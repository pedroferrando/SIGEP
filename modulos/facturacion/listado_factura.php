...
$sql_tmp = "SELECT 
  ...
  facturacion.factura.ctrl,
  facturacion.factura.tipo_nomenclador
FROM
  facturacion.factura
  LEFT JOIN facturacion.smiefectores using (cuie)";

$id_usuario = $_ses_user['id'];
$cuieses = traeCuiesPorUsuario($id_usuario);

if ($cmd == "A") {
    if ($id_usuario)
        $where_tmp = " (factura.estado='A') and cuie in $cuieses";
    else
...
if ($cmd == "C") {
    if ($id_usuario)
        $where_tmp = " (factura.estado='C') and cuie in $cuieses";
    else
...
if ($cmd == "X") {
    if ($id_usuario)
        $where_tmp = " (factura.estado='X') and cuie in $cuieses";
    else
...
<form name=form1 action="listado_factura.php" method=POST>
        ...
        <tr>
            <td align=center>
                ...
                if ($_POST['keyword']) {
                    ...
                    $sql_tmp = "";
                    list($sql, $total_muletos, $link_pagina, $up) = form_busqueda($sql_tmp, $orden, $filtro, $link_tmp);
                }
                ?>
    ...
    <table border=0 width=100% cellspacing=2 cellpadding=2 bgcolor='<?= $bgcolor3 ?>' align=center>
        ...
        <tr>
            ...
            <td align=right id=mo><a id=mo href='<?= encode_link("listado_factura.php", array("sort" => "6", "up" => $up)) ?>'>Periodo</a></td>
            <td align=right id=mo><a id=mo >Tipo de Nomenclador</a></td>
        ...
        while ((!$result->EOF) && ($result != null)) {
            $ref = encode_link("factura_admin.php", array("id_factura" => $result->fields['id_factura']));
            ...
            <tr <?= atrib_tr($colordefondo) ?>>     
                <td onclick="<?= $onclick_elegir ?>"><?= $func_nroFactura ? $result->fields['numero_factura'] : $result->fields['id_factura'] ?></td>
                <?
...
//                }
                ?>   
                <?
                if ($result->fields['ctrl'] == "S") {
                    $controlado = "CONTROLADO";
                } else {
                    $controlado = "SIN CONTROLES";
                }
                ?>
                <td onclick="<?= $onclick_elegir ?>" title="<?= $title ?>"><?= $result->fields['cuie'] ?></td>
                ...
                <td onclick="<?= $onclick_elegir ?>" ><?= $result->fields['tipo_nomenclador'] ?></td> 

                <?
                if ($controlado == "SIN CONTROLES") {
                    $color = '#FA5858';
                } else {
                    $color = '#81F781';
                }
                ...
                <td onclick="<?= $onclick_elegir ?>" bgcolor="<?= $color ?>"><? echo $controlado ?></td> 
                <?
                if ($cmd == "C") {
		  ...
		  WHERE factura.id_factura=$id_factura";
                    ...
                    $query_t1 = "SELECT sum(nomenclador.prestaciones_n_op.precio) as total1
		   FROM facturacion.factura
		   INNER JOIN facturacion.comprobante ON (facturacion.factura.id_factura = facturacion.comprobante.id_factura)
        ...
        <tr>
            <td width=30% bordercolor='#FFFFFF'>
                <table border=1 bordercolor='#FFFFFF' cellspacing=0 cellpadding=0 width=100%>
                    <tr>
                        <td width=30 bgcolor='#FA5858' bordercolor='#000000' height=30>&nbsp;</td>
                        <td bordercolor='#FFFFFF'>Factura Sin Proceso de Control</td>
                    </tr> 
                    <tr>
                        <td width=30 bgcolor='#81F781' bordercolor='#000000' height=30>&nbsp;</td>
                        <td bordercolor='#FFFFFF'>Factura Controlada</td>
                    </tr>       
                </table>
            </td>
    </table>
