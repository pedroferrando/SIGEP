...
INNER JOIN facturacion.recepcion r on r.idrecepcion=facturacion.factura.recepcion_id
  left join facturacion.smiefectores using (cuie) 
  where id_factura='$id_factura' ";

$result1 = $db->Execute($query) or die($db->ErrorMsg());

$querysolonomenclador = "SELECT modo_facturacion 
                        FROM facturacion.comprobante
			INNER JOIN facturacion.prestacion using (id_comprobante)
			INNER JOIN facturacion.nomenclador r using (id_nomenclador)
                        INNER JOIN facturacion.nomenclador_detalle d on r.id_nomenclador_detalle=d.id_nomenclador_detalle
                        where id_factura='$id_factura' and prestacion.id_nomenclador > 0";
...
?> 
    ...
    $q = "SELECT p.id_nomenclador,n.codigo,p.precio_prestacion precio,
                    p.id_anexo, a.numero,  sum(p.cantidad) cantidad
                        ...
                        where c.id_comprobante NOT IN(SELECT id_comprobante 
                                                    FROM facturacion.debito 
                                                    WHERE id_factura='$id_factura')
                        AND c.id_factura='$id_factura'
                    GROUP BY p.id_nomenclador,n.codigo,p.precio_prestacion,p.id_anexo,a.numero
    ...
    <table width="100%" align=center border=1 bordercolor=#585858 cellspacing="0" cellpadding="5">
        ...
        if ($otro_result->RecordCount() != 0) {
            while (!$otro_result->EOF) {
                ...
                if ($otro_result->fields['id_anexo'] > 0) {
                    $codigomuestra = $codigoarmado . " - " . $otro_result->fields['numero'];
                    $numero = $otro_result->fields['numero'];
                    $consultanumero = "AND a.numero =$numero";
                } else {
                    $consultanumero = "";
                    $codigomuestra = $codigoarmado;
                }

                $codigocss = str_replace(" ", "", $codigomuestra);
                //$total = $otro_result->fields['sumatotal'];
                ?>
                ...
                $detalle_nomenclador = $codigoarmado;
                $query = "SELECT c.id_comprobante,to_char(c.fecha_comprobante,'YYYY-MM-DD') as fecha_comprobante,p.precio_prestacion*p.cantidad suma,
                            ...
                            WHERE id_comprobante NOT IN(SELECT id_comprobante FROM facturacion.debito WHERE id_factura='$id_factura')
                            AND c.id_factura='$id_factura'
                            AND REPLACE(n.codigo, ' ', '') =REPLACE('$detalle_nomenclador', ' ', '')
                            " . $consultanumero;
