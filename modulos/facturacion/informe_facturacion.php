...
if ($func_nroFactura) {
  $filtro = array(
      "(case when nro_fact_offline <> ''
  then cast(nro_fact_offline as text)
  else cast(id_factura as text) end)" => "Nro. Factura",
      "cuie" => "CUIE",
      "nombreefector" => "Efector"
  );
}
...
FROM
  ...
  INNER JOIN facturacion.recepcion ON (facturacion.recepcion.idrecepcion = facturacion.factura.recepcion_id)";
...
?>
    ...
    <tr <?= atrib_tr() ?>>
      <td ><?= $func_nroFactura ? $result->fields['numero_factura']
          : $result->fields['id_factura'] ?></td>
      ...
      <? (number_format($total_a_pagar, 2, ',', '.') == number_format($total, 2, ',', '.'))
              ? $bg_color = "" : $bg_color = "FF9999"; ?>
        ...
        <td >
        <?php
        echo $result->fields['cod_org'] . '-' . $result->fields['no_correlativo'] . '-' . $result->fields['ano_exp'] . '-' . $result->fields['cuerpo'];
        ?></td>
