...
//imprime el cuadro donde se veran las practicas
        $pdf->facturaCabecera();
        $practicasdebitadasdelafactura = practicasEnDebito($factura->getIdFactura());
...
//imprime una linea, correspondiente a un codigo de practica de la factura totalizado.
            $codigo_deb = $practicasdebitadasdelafactura->fields['codigo_deb'];
            $mensaje_baja = $practicasdebitadasdelafactura->fields['mensaje_baja'];
            $documento_deb = $practicasdebitadasdelafactura->fields['documento_deb'];
            $apellido_deb = $practicasdebitadasdelafactura->fields['apellido_deb'];
            $nombre_deb = $practicasdebitadasdelafactura->fields['nombre_deb'];
            $cantidad = number_format($practicasdebitadasdelafactura->fields['cantidad'], 0, ',', '.');
            $monto = number_format($practicasdebitadasdelafactura->fields['monto'], 2, ',', '.');
            $total = number_format($practicasdebitadasdelafactura->fields['cantidad'] * $practicasdebitadasdelafactura->fields['monto'], 2, ',', '.');
            $pdf->facturaFila($codigo_deb, $mensaje_baja, $documento_deb, $apellido_deb, $nombre_deb, $cantidad, $monto, $tiponomenclador);
