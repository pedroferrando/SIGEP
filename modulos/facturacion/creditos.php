...
// Ordenada de comienzo de la columna
    ...
    function Header() {
        ...
        $this->MultiCell(20, 2, 'Programa SUMAR - Misiones', 0, 'C');
        $this->Ln(5);
    }
    ...
    function facturaCabecera() {
        ...
        $this->Cell(20, 4, "DNI", 1, 0, 'C');
        $this->Cell(40, 4, "Apellido", 1, 0, 'C');
        $this->Cell(40, 4, "Nombre", 1, 0, 'C');
        $this->Cell(10, 4, "Cant.", 1, 0, 'C');
        $this->Cell(20, 4, "Valor U.", 1, 0, 'C');
        $this->Cell(20, 4, "Total", 1, 0, 'C');
        $this->Cell(20, 4, "Fecha", 1, 0, 'C');
        $this->Ln();
    }

    function facturaFila($codigo_deb, $documento_deb, $apellido_deb, $nombre_deb, $monto, $cantidad, $fecha, $tiponomenclador) {
        $this->SetFont('TIMES', '', 6);
        $this->SetX(15);
        $this->Cell(20, 4, $codigo_deb, 1, 0, 'C');
        $this->SetX(35);
        //$this->Cell(80, 4, $mensaje_baja, 1, 0, 'L');
        $this->Cell(20, 4, $documento_deb, 1, 0, 'C');
        $this->Cell(40, 4, $apellido_deb, 1, 0, 'C');
        $this->Cell(40, 4, $nombre_deb, 1, 0, 'C');
        $this->Cell(10, 4, $cantidad, 1, 0, 'C');
        $total = $cantidad * $monto;
        if (($tiponomenclador != "PERINATAL_CATASTROFICO") && ($tiponomenclador != "PERINATAL_NO_CATASTROFICO")) {
            $this->Cell(20, 4, "$ " . number_format($monto, 2, ",", "."), 1, 0, 'C');
        } else {
            $this->Cell(20, 4, "-", 1, 0, 'C');
        }

        $this->Cell(20, 4, "$ " . number_format($total, 2, ",", "."), 1, 0, 'C');
        $this->Cell(20, 4, substr($fecha, 0, 10), 1, 0, 'C');
        $this->Ln();
    }
    ...
    function totalDelEfector($monto_debitado, $cantidad) {
        ...
        $this->Cell(40, 4, "Total del Efector " . $this->cuie, 1, 0, 'C', 1);
        $this->Ln();
        $this->SetX(-55);
