...
<table border=0 width=100% cellspacing=2 cellpadding=2 bgcolor='<?=$bgcolor3?>' align=center>
  ...
  <tr>
    ...
    <td align=right id=mo><a id=mo href='<?=encode_link("listado_beneficiarios_fact.php",array("sort"=>"4","up"=>$up))?>'>Tipo Beneficiario</a></td>
    ...
    <?if (($cmd=="todos")||($cmd=="inactivos")){?>
    	<td align=right id=mo><a id=mo href='<?=encode_link("listado_beneficiarios_fact.php",array("sort"=>"8","up"=>$up))?>'>Activo</a></td>
    ...
    <td align=right id=mo><a id=mo href='<?=encode_link("listado_beneficiarios_fact.php",array("sort"=>"10","up"=>$up))?>'>F Carga</a></td>
    <?if ($cmd=="inactivos"){?>    	
   ...
   while (!$result->EOF) {
    ...
    <tr <?=atrib_tr()?>>     
     ...
     <?}?>    
      ...
      <td onclick="<?=$onclick_elegir?>"><?=fecha($result->fields['fechacarga'])?></td>
      <?$m_b=$result->fields['motivobaja'];
      	...
      	if ($m_b=='22'||$m_b=='31'||$m_b=='41'){
    		...
    		?>
      		<td title="Factura Excepciones (Embarazadas - Puerperas con ciclo cumplido y Niños con 6 años Cumplidos)" onclick="<?=$onclick_elegir1?>" align="center"><img src='../../imagenes/luz1.gif' style='cursor:pointer;'></td>
		    <?}
		else{?>
		   <td title='No hay motivo para facturar este item' align="center" onclick="alert ('No hay motivo para TENER QUE FACTURAR este item - Si encuentra un motivo avise a Facturacion.')"><img src='../../imagenes/salir.gif' style='cursor:pointer;'></td>
		<?}
      	}?>     	
