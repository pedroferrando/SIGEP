...
if ($_POST['guardar']=='Guardar'){
   ...
   $query="insert into facturacion.validacion_prestacion
   	(id_val_pres, codigo, cant_pres_lim, per_pres_limite, msg_error)
   	values
  	('$id_val_pres', '$codigo', '$cant_pres_lim', '$per_pres_limite', '$msg_error')";
...
$res_val=sql($query, "Error al traer el Comprobantes") or fin_pagina();

$codigo=$res_val->fields['codigo'];
...
{ 
 if(document.all.codigo.value==""){
  alert('Debe ingresar una codigo');
  return false;
 }
  if(document.all.cant_pres_lim.value==-1){
  alert('Debe ingresar la cantidad de prestaciones');
  return false;
 }  
 if(document.all.per_pres_limite.value==-1){
     ...
     <tr>
       <td>
        ...
        <tr>
            ...
            <td align='left'>
			              <select name=codigo Style="width=650px" <? if($id_val_pres) echo "disabled"?> >
			              <option value=-1>Seleccione</option>
			              <?$sql= "SELECT distinct codigo, descripcion FROM  facturacion.nomenclador 
        							where codigo!='DIFERENCIA DE NOMENCLADOR' and codigo!='DEB-CRED'
        							order by codigo";
												$res_sql=sql($sql) or fin_pagina();
												while (!$res_sql->EOF){ 
												$codigo_aux=$res_sql->fields['codigo'];
												$codigo_sql=$res_sql->fields['codigo'].'- '.$res_sql->fields['descripcion'];
												?>
