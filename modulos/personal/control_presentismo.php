...
function muestra_tabla(obj_tabla, nro){
					...
					<tr>
						<td align=center>
							<?
								...
								if ($ubicacion){
									...
									if (($_POST["sel_ubicacion"])&&($_POST["sel_ubicacion"]!="Todas")){
										...
										$where_tmp.="and(d.nombre ilike '%Plan Nacer%')";
									}else{
										$where_tmp.="and(1=1)";
									}
								}
							...
							<select id="sel_ubicacion" name="sel_ubicacion">
							  <option value='Plan Nacer' <?if ($ubic=='Plan Nacer') echo 'selected'?>>Plan Nacer</option>
	...
	<table border=0 width=95% cellspacing=2 cellpadding=3 bgcolor='<? echo $bgcolor3; ?>' align=center>
		<tr>			
			<input type="submit" name="stat_sl" value="Plan Nacer"></input>
		</tr>
<?
	if (($_POST["stat_bsas"])||($_POST["stat_sl"])||($_POST["actualizar"])||($parametros["dia_seleccionado"])){
?>
				...
				<tr>
					<?
						...
						for ($i=1; $i<count($asistencia); $i++){
							...
							echo "<td align='center' bgcolor='$colorDia' style='cursor:pointer;' onclick=\"document.location.href='".encode_link("control_presentismo.php",array("ubicacion"=>$ubicacion, "dia_seleccionado"=>$i, "mes_resumen"=>$mes_resumen, "agno_resumen"=>$agno_resumen))."'\"><font color=".contraste($colorDia, '#0000FF', '#FFFFFF')."><b>$i</b><font></td>";
						}
					?>
				</tr>
			</table>
...
?>
						...
						<td style="cursor:pointer;" bgcolor='<?=$bgcolor3?>' onclick="muestra_tabla(document.all.tabla_expandible,1);">
...
?>
	...
	<?   
			...
			for ($i=0; $i<count($lista_nombres); $i++){
			 ...
			 if ($ubic=="") $ubic_personal="Plan Nacer";
