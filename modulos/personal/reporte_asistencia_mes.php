...
$ubicacion=$parametros['ubicacion'] or $ubicacion=$_POST['ubicacion'] or $ubicacion='Plan Nacer';
...
<table border="1" cellpadding="0"  align="center" cellspacing="0">
 <tr>
   ...
   <? 
     ...
     ?>
      <tr>
        <td id=ma style="cursor:pointer" onclick="window.open('<?=encode_link('reporte_asistencia.php',array("id_usuario"=>$id_usuario))?>', '', '');">
           <?=$nombre?></td>
        <?
        $tardanza_injust=0;
        $tardanza_just=0;
        	...
        	elseif (($datos[$k]['fecha']=="$agno_resumen-$mes_resumen-$dia")&&($datos[$k]["tipo"]==1)){
        		...
        		$k++;
                 ...
                 if ((g_timeToSec($datos[$k]["hora_entra"])) > ($hora_entra + g_timeToSec("00:15:00"))) {
        		     ...
        		     $tardanza_injust++;
        		     $onclick="onclick=\"window.open('".encode_link("justificar_inasistencia.php",array("id_usuario"=>$id_usuario, "tipo"=>"2","nombre"=>$nombre,"fecha"=>"$agno_resumen-$mes_resumen-$dia"))."','','');\" style=\"cursor:pointer\"";
        		...
        		$falta_injust++;
        		$onclick="onclick=\"window.open('".encode_link("justificar_inasistencia.php",array("id_usuario"=>$id_usuario, "tipo"=>"1","nombre"=>$nombre,"fecha"=>"$agno_resumen-$mes_resumen-$dia"))."','','');\" style=\"cursor:pointer\"";
